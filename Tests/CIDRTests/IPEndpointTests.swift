...
struct IPEndpointTests {
    ...
    @Test("Port stores the raw UInt16 value")
    func portStoresRawValue() {
        let port = Port(443)
    ...
    func ipEndpointStoresAddressAndPort() throws {
        ...
        let endpoint = IPEndpoint(address: address, port: Port(53))

        #expect(endpoint.address == address)
        #expect(endpoint.port.rawValue == 53)
    }

    @Test("Port and IPEndpoint round-trip through Codable")
    func portAndEndpointCodableRoundTrip() throws {
        let address = try #require(IPv6Address("2001:db8::1/64"))
        let endpoint = IPEndpoint(address: address, port: Port(443))

        let encodedPort = try encoder.encode(Port(443))
